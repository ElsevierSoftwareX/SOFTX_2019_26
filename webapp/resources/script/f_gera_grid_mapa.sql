CREATE OR REPLACE FUNCTION public.f_gera_grid_mapa(
    p_codarea integer,
...
BEGIN
    ...
    -- EXECUTE ' SELECT AddGeometryColumn('''||P_Mapa||''', ''geometry'', '||P_SridDefaultBanco||', '''||P_Tipogeometria||''', 2)'; 
      ...
      FOR V_y IN 0..V_nPixel_y LOOP
          FOR V_x IN 0..V_nPixel_x LOOP
			  ...
			  IF P_Tipogeometria = 'POINT' THEN  
					  V_str_Insert := 'insert into '||P_Mapa||'(geometry, mapa_id) values '||
	  ...
	  V_str_Delete := 'delete from '||P_Mapa||' b where b.mapa_id = '|| p_codgrade ||' and b.id not in (select a.id from '||P_Mapa||' a, '||p_nometabelaareas||' c
            where (st_Intersects( c.geometry, a.geometry) or st_contains( c.geometry, a.geometry) ) and c.id = '||P_Codarea||')';
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION public.f_gera_grid_mapa(integer, text, double precision, double precision, text, integer, text, integer)
  OWNER TO postgres;