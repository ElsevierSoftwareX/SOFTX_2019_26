-- Function: public.f_gera_grid(integer, text, double precision, double precision, text, integer, text, integer)

-- DROP FUNCTION public.f_gera_grid(integer, text, double precision, double precision, text, integer, text, integer);

CREATE OR REPLACE FUNCTION public.f_gera_grid(
    p_codarea integer,
...
DECLARE
  ...
  v_latcentral float;
BEGIN
      execute 'select st_x(st_centroid(geometry)), st_y(st_centroid(geometry)) from '||p_nometabelaareas|| '
      ...
      EXECUTE 'Select st_xmin(st_transform(geometry,'||local_srid||')), st_xmax(st_transform(geometry,'||local_srid||')),  st_ymin(st_transform(geometry,'||local_srid||')), st_ymax(st_transform(geometry,'||local_srid||')) from '||P_NomeTabelaAreas||' where id = '||P_Codarea INTO V_xmin, V_xmax, V_ymin, V_ymax;
    ...
    -- EXECUTE ' SELECT AddGeometryColumn('''||P_Mapa||''', ''geometry'', '||P_SridDefaultBanco||', '''||P_Tipogeometria||''', 2)'; 
      ...
      FOR V_y IN 0..V_nPixel_y LOOP
          FOR V_x IN 0..V_nPixel_x LOOP
			  ...
			  IF P_Tipogeometria = 'POINT' THEN  
					  V_str_Insert := 'insert into '||P_Mapa||'(geometry, amostra_id) values '||
	  ...
	  V_str_Delete := 'delete from '||P_Mapa||' b where b.id not in (select a.id from '||P_Mapa||' a, '||p_nometabelaareas||' c
            where (st_Intersects( c.geometry, a.geometry) or st_contains( c.geometry, a.geometry) ) and c.id = '||P_Codarea||' and a.amostra_id = '|| p_codGrade ||')';
      
	raise notice 'TESTE TESTE TESTE : % ', V_str_Delete;	
...
$BODY$
  ...
  COST 100;
ALTER FUNCTION public.f_gera_grid(integer, text, double precision, double precision, text, integer, text, integer)
  OWNER TO postgres;
